...
VALUES 
	...
	select id ,shops.shopname , cats.fk_cats_shops_id from cats
    join shops on shops.id = cats.id;
    
    


SELECT shopname
FROM shops -- teacher = t, левая таблица
JOIN cats --  lesson = l, правая таблица
ON cats.id = shops.id 
...
JOIN cats --  lesson = l, правая таблица
ON cats.id = shops.id 
WHERE cats.name != "Murzik" and "Zuza";
...
VALUES 
    ...
    join groupsan  
     on Analysis.an_id = groupsan.gr_id
