...
select users.id , users.firstname , users.lastname , profiles.hometown , profiles.photo_id
from users
join profiles on users.id = profiles.user_id;


-- выбрать фотографии (filename) пользователя с email: arlo50@example.org . 
select media.filename , users.id
from media
...
from friend_requests where initiator_user_id or target_user_id = '1' and 'status' = 'approved'
union  select users.firstname, users.lastname
from users where friend_requests.target_user_id = users.id ;

-- Список медиафайлов пользователей, указав название типа медиа (id, filename, name_type)
select users.id, media.filename,media_types.name_type
from users
left join media on media.user_id = users.id
join media_types on media.media_type_id = media_types.name_type;


-- Получите друзей пользователя с id=1
create view friends_yeah as 
select *
from friend_requests
where initiator_user_id or target_user_id = '1' and 'status' = 'approved' ;

-- Создайте представление, в котором будут выводится все сообщения, в которых принимал
-- участие пользователь с id = 1.
create view message_yeah as
select *
from messages
where from_user_id or to_user_id = '1';

-- Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u)

create view likes_yeah as
select users.id, users.firstname ,users.lastname , likes.user_id , likes.media_id, media.filename,
count(likes.media_id) over (PARTITION bylikes.media_id) as like_count	
-- возможно напутал с порядком операций, но с точки зрения логики всё нормально...
    -- не получается применять разделение...выводит ошибку(не понимаю почему)
    -- он пишет ошибку на точку...я не знаю уже совсем чего-тут делать

from users
join likes
on likes.user_id = users.id
join media				-- проблема в том что нужно посчитать лайки
...
create view groups_yeah as
select users.id, users.firstname ,users.lastname ,count(communities.id)
from users
join communities
on communities.id = users.id;

-- Вывести 3 пользователей с наибольшим количеством лайков за медиафайлы
-- create view TOP3_yeah as
-- чтобы сделать задание здесь необходимо посчитать лайки, что в более раннем задании у меня не получилось
-- но схематически что сделать я понимаю
-- нужно повторить алгоритм счёта лайков и задать
-- limit(3)
